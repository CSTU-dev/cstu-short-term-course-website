...
/** Password-reset link. Sent only to accounts that have a password set. */
export function passwordResetEmail(resetUrl: string): EmailContent {
  return {
    subject: "Reset your CSTU password",
    html: layout(`
      <p>We received a request to reset your CSTU password. Click below to choose a new one:</p>
      <p>${button(resetUrl, "Reset password")}</p>
      <p style="font-size:13px;color:#666">Or paste this link into your browser:<br><a href="${resetUrl}">${resetUrl}</a></p>
